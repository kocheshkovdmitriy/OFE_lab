...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('letter', type=str, help='Литер класса')
    def handle(self, *args, **options):
        print('Run custom command!')
        grade = self.__get_class(options['class'])
        letter = self.__get_letter(options['letter'])

        if not (grade and letter):
    ...
    def __get_letter(self, letter: str):
        return models.Letter.objects.filter(name=letter).first()
